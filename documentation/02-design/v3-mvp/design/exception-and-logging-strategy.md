...
| Fetching with an empty/malformed short-code segment | `ShortUrlResolverService.ResolveAsync` short-circuits null/whitespace codes to `NotFound` without a cache/DB round trip; `RedirectController` rejects a non-empty code containing characters outside the generator's base62 alphabet with `400 Bad Request`, before any cache/DB round trip (`api-design.md` §2 item 0, §6.6 hardening addition) | `ShortUrlResolverServiceTests.ResolveAsync_WithEmptyOrWhitespaceCode_ReturnsNotFoundWithoutQueryingCacheOrRepository` (`[Theory]`: `""`, `"   "`); `ShortUrlsEndpointTests.Fetch_WithEmptyCodeSegment_ReturnsNotFound`, `Fetch_WithMalformedCodeSegment_ReturnsBadRequest` |
