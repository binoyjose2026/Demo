...
- No formal contractual SLA exists for v1 (per `00-getting-started/02-in-scope/01-summary.md`, Section H — best-effort standard, no formal SLA). The numbers above are the **internal design target** this document is built against, not a customer-facing commitment.
...
- **Reconciliation note — not the interface `fn-fetch.md` (and the shipped code) actually use.** `fn-fetch.md` §5 and `nfr-scalability.md` §2's caching decorator both define `IShortUrlRepository.GetByCodeAsync(string code) : Task<ShortUrl?>` — returning the full entity, not the `FindRedirectTargetAsync`/`ShortUrlRedirectTarget` projection proposed above — and that is the interface shape that actually shipped (`src/UrlShortener.Domain/Repositories/IShortUrlRepository.cs`). This section's projection is this document's own proposed micro-optimization for the over-fetching concern it raises; it was not adopted when `fn-fetch.md`'s interface was finalized, and `ShortUrl` is small enough in practice that the full-entity fetch was judged not to cost enough to justify a second, narrower repository contract. Left here as a documented, not-taken optimization path (the numbers/rationale above remain valid if over-fetching ever does become measurable) rather than silently deleted, per this project's convention of naming trade-offs instead of hiding them.
...
| 4 | **Exception, not adopted:** `FindRedirectTargetAsync` returns a non-entity projection record, not a `Domain` entity — proposed here, but `fn-fetch.md`'s finalized `IShortUrlRepository.GetByCodeAsync` (also what shipped) returns the full entity instead; see Section 4's reconciliation note | ANFR-05, deviates from `design-guidelines.md` §2 (rationale in Section 4) |
