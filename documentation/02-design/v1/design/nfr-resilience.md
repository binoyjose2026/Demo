...
> - **Health-check endpoints / readiness probes** (`/health`, `/health/ready`) — not wired up in v1; there is no orchestrator (Kubernetes, load balancer) in the current deployment model consuming them yet. Adding `Microsoft.Extensions.Diagnostics.HealthChecks` is a low-cost addition to make once a hosting/deployment design document defines what would consume the probe.
