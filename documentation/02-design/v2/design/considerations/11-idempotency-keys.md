...
- **Generated once, reused across every retry of the same attempt.** If the client's resilience wrapper (`15-retry.md`) retries a request three times with backoff, all three carry the *same* `Idempotency-Key` — that's what makes them recognizable as "the same logical thing" rather than three independent creates.
