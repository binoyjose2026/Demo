...
**Traceability:** `agent-prompt.md` ("Use CDN, Firewall, Load Balancer and worker computers and use Kubernetis elastic model from azure so server capacity will increase automatically. Create an infrastructure design for this").
...
- `05-kafka-comparison.md` — message-broker recommendation (Azure Service Bus Topics/Subscriptions as default; Kafka as a named upgrade path).
...
- **Cost estimation.** No dollar figures, no reserved-instance/spot-VM pricing analysis, no comparison of Azure SKU pricing tiers. `07-redis-caching-and-invalidation.md` and `05-kafka-comparison.md` both flag their own sizing numbers ("4gb" Redis budget, node counts) as illustrative starting points to be tuned from observed data, not costed commitments — the same caveat applies to every number in this document (HPA thresholds, node pool sizes, Cluster Autoscaler min/max bounds).
