...
- `05-kafka-comaporison.md` — establishes the broker consumers (analytics-indexing into Elasticsearch, cache-invalidation, and a candidate future async malicious-domain re-check) as long-lived, stateful consumer-group workloads, and recommends Azure Service Bus Topics/Subscriptions as the default broker.
...
| Broker consumer: analytics-indexing (writes to Elasticsearch) | `05-kafka-comaporison.md` §1, `03-elasticsearch-vs-sql-server.md` | Long-lived consumer-group member, continuous stream, ~1,200 events/sec average / ~6,000–12,000/sec peak at 5-year scale (`05-kafka-comaporison.md` §2.3) |
...
| Broker consumer: async malicious-domain re-check (candidate, not yet committed) | `05-kafka-comaporison.md` §6 | Long-lived consumer-group member if adopted; per-event work, not high-frequency |
...
- The broker consumers exist specifically so that create/fetch requests never block on analytics indexing or cache invalidation (`05-kafka-comaporison.md` §1: "producers... should never block on... a downstream consumer being slow"). A consumer taking an extra second to cold-start doesn't delay the HTTP response that already returned.
...
1. **Long-running Worker Service (Option A) for:**
   ...
   - The candidate future async malicious-domain re-check consumer (`05-kafka-comaporison.md` §6), if adopted.
