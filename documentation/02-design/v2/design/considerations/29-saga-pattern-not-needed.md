...
**Status:** Draft
**Scope:** This document answers exactly one question — does the create-short-URL flow need the Saga pattern to coordinate its side effects? It builds on the same "don't over-engineer the create path" reasoning already established in `20-outbox-pattern.md` (which concluded the Outbox pattern is not needed for `UrlCreated`), and applies it to a different, larger pattern: distributed-transaction coordination via Sagas. Builds on the v1 create flow in `../../v1/design/fn-create.md` and the async event pipeline in `05-kafka-comaporison.md`.
